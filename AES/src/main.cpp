...
std::vector<std::vector<unsigned char>> clave_, texto_, texto_cf_;
...
RecibirEntrada() {
    ...
    std::cout<<"Introduzcla la clave 16 bytes en hexadecimal"<<std::endl; // Litle Endian
	...
	std::cout<<"Bloque de Texto Original (16 bytes en hexadecimal)"<<std::endl;
    std::getline(std::cin,str);
...
int main() {
	...
	for (int i=0; i<4; i++){
		for (int j=0; j<4; j++)
			std::cout << std::hex << std::setfill('0') << std::setw(2) << int(clave_[j][i]);
	}
	std::cout << std::endl << "Bloque Bloque de Texto Original: ";
	for (int i=0; i<4; i++){
		for (int j=0; j<4; j++)
			std::cout << std::hex << std::setfill('0') << std::setw(2) << int(texto_[j][i]);
	}
    encriptar();
}
